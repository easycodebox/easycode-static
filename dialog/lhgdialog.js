...
_args, _path = (function( script, i, me )
{
	...
	for( ; i < l; i++ )
	{
		...
		//为了兼容http://www.xxx.com??js/a.js,js/lhgdialog.js,js/b.js格式
	    if( me.indexOf('lhgdialog') !== -1 )
		    break;
	}
	if(new RegExp("^([^\\?]*)\\?\\?.*?([^,]*lhgdialog)").test(me)) {
		_args = me.substring(me.lastIndexOf('?') + 1);
		return RegExp.$1 + RegExp.$2.substring(0, RegExp.$2.lastIndexOf('/') + 1);
	}else {
		me = me.split('?');
		_args = me[1];
...
var lhgdialog = function( config )
{
	...
	if(utils.isBoolean(config.before) && !config.before)
		return;
	else if(utils.isString(config.before) && config.before.trim().toLowerCase() !== "true")
		return;
	else if(utils.isFunction(config.before)) {
		if(!config.before.call(config))
			return;
	}
	
	//获取ajax内容
    if(_rurl.test(config.content) && config.noFrame) {
    	var url = config.content.split('url:')[1];
    	config.content = null
		// 是否允许缓存. 默认true
		...
		$.ajax({
			...
			type: "post",
			url: url,
			data: config.data ? utils.isFunction(config.data) ? 
					config.data.call(config) : config.data : null,
			suc: function(data){
				config.content = data;
			}
		});
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	content: function( msg, add, frm )
	{
		...
		if( frm ) {
			...
			$content.html( typeof msg == "object" ? (that.config._$content = $(msg).show()) : msg );
		}
	...
	close: function()
	{
		...
		if(that.config._$content) {
			$("body").append(that.config._$content.hide());
			that.config._$content = null;
		}
...
lhgdialog.setting =
{
    ...
    _$content: null,			//自动复制传入的内容对象
    noFrame: true,				// content中包含url也不用iframe加载
	title: '\u89C6\u7A97 ',     // 标题,默认'视窗'
	button: null,	     		// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数
	before: null,				// 显示对话框之前执行的函数，只有 return ture 才显示对话框 
...
/**
 * 弹出框信息
 * @param code ==> warn/info/error (可以不传)
 * @param	{String}	消息内容
 */
lhgdialog.alert = function( code, content, callback, parent )
{
	var msgType = {
			info: {title: "提示", icon: "success.gif"},
			error: {title: "错误", icon: "error.gif"},
			fail: {title: "错误", icon: "error.gif"},
			warn: {title: "警告", icon: "alert.gif"}
		},
		title = "提示",
		icon = null;
	if(content == null || utils.isFunction(content)) {
		parent = callback;
		callback = content;
		content = code;
	}else {
		title = msgType[code].title;
		icon = msgType[code].icon;
	}
	return lhgdialog({
		title: title,
		id: 'Alert',
