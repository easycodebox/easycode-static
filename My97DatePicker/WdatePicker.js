 ...
 * J()获取js请求的根路径被兼容Tengine的合并功能：httP://xxx.com/??a.js,b.js
 * function J() {
		var _, $ = Y[N][C]("script");
		for (var B = 0; B < $.length; B++) {
			var src = $[B].getAttribute("src") || "";
			if(src.toLowerCase().indexOf("wdatepicker") > -1) {
				if(new RegExp("^([^\\?]*)\\?\\?.*?([^,]*wdatepicker)", "i").test(src)) {
					_ = RegExp.$1 + RegExp.$2.substring(0, RegExp.$2.lastIndexOf('/') + 1);
				}else {
					_ = src.substr( 0, src.lastIndexOf('/') + 1 );
				}
				break
			}
		}
