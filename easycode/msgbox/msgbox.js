...
;(function($){
	...
	utils.loadCss(BaseData.path + "/js/msgbox/msgbox.css");
	var boxDom = ['<div class="msg_box" style="display: none;">',
						'<div class="title_div">',
					        ...
					        '<div class="title_center">',
					            ...
					            '</div>',
					            '<div class="msg_close">',
					            	'<img src="' + BaseData.path + '/js/msgbox/imgs/pic_guanbi.gif" />',
					            '</div>',
					        '</div>',
	...
	$.extend({msgbox: function(settings){
		...
		if(settings.wordsNum)
			$msg_con.text(settings.content.textCut(settings.wordsNum, "...", false))
		...
		if(settings.detailUrl) {
			var detail = ['<div class="lookup_btn" style="width:100%; margin-top:10px; height:17px">',
								'<div style="width:40px; height:17px; float:left;text-align:right;">',
									'<a class="msg_ignore_msg" href="javascript:">忽略</a>',
								'</div>',
					         	'<div style="width:85px; height:17px; float:right;">',
					            	'<div class="fdj">',
					                '</div>',
					                '<div style="width:66px; height:17px; float:left">',
					                	'<a class="open_detail"  href="javascript:"><span style="margin-left:3px">点击查看</span></a>',
					                '</div>',
					            '</div>',
					         '</div>'].join("");
			$boxDom.find(".msg_content").after(detail);
			$boxDom.find(".open_detail").click(function(){
				window.frames["mainIframe"].location = settings.detailUrl;
				$boxDom.find(".msg_close").click();
			});
		}
		...
		$boxDom.find(".msg_ignore_msg").click(function(){
			...
			if(settings.msgId){
				$.get(BaseData.path + "/message/readMsg.json",{id: settings.msgId});
			}
			$boxDom.animate({"bottom": "-" + $boxDom.height() + "px"},"slow", function(){
				$(this).remove();
			})
