...
;(function($){
	...
	window.utils = {
		...
		//mode == add || remove; mode默认为add; rule == r100c100
		imgUrlFormat : function(url, rule, mode) {
			if(!url || (url = $.trim(url)) === "" || !rule) return url;
			mode = mode ? mode.toLowerCase() : 'add';
			if(mode === 'remove'){
      ...
      //转换数据为int类型，NaN返回defaultVal，defaultVal默认为0
		...
		addUnit: function (val, unit) {
			unit = unit || 'px';
			return val && new RegExp(regex.number).test(val) ? val + unit : val;
		},
		//删除单位
...
//			}
        ...
        submit: {
        	...
        	closeDialog: function() {
        		console.warn("utils.submit.closeDialog没有实现！！！");
        	}
        },
	...
	window.regex = {
		...
		number:"^([\\+-]?)\\d*\\.?\\d+$",	//浮点数 或 整数(包含正负数)
		number_plus:"^\\+?((\\d*[1-9]\\d*\\.?\\d*)|(0+\\.\\d*[1-9]\\d*))$",//正小数、正整数
		number_minus:"^-((\\d*[1-9]\\d*\\.?\\d*)|(0+\\.\\d*[1-9]\\d*))$",//负小数、负整数
		number_no_plus:"^-\\d*\\.?\\d+$",	//非正小数、非正整数
		number_no_minus:"^\\+?\\d*\\.?\\d+$",//非负小数、非负整数
		
		decemal:"^([\\+-]?)\\d*\\.\\d+$",	//只能是浮点数不能为整数(包含正负数)
		decemal_plus:"^\\+?((\\d*[1-9]\\d*\\.\\d*)|(0+\\.\\d*[1-9]\\d*))$",//正小数
		decemal_minus:"^-((\\d*[1-9]\\d*\\.\\d*)|(0+\\.\\d*[1-9]\\d*))$",//负小数
		decemal_no_plus:"^-\\d*\\.\\d+$",	//非正小数
		decemal_no_minus:"^\\+?\\d*\\.\\d+$",//非负小数
		...
		empty: "^\\s*$",						//空白
		not_empty:"^\\S+$",						//非空
		image:"(.*)\\.(jpg|bmp|gif|ico|pcx|jpeg|tif|png|raw|tga)$",	//图片
	 ...
	 * $.ajax({
            ...
            submiting: "保存..." //【可选】正在请求中的提示信息，格式可以用占位符：{0}...
        });
	 */
    ...
    $.ajaxSetup({
    	...
    	error: function(XMLHttpRequest, textStatus, errorThrown){
		    ...
		    if (XMLHttpRequest.responseJSON) {
			    var code = XMLHttpRequest.responseJSON[BaseData.code],
				    msg = XMLHttpRequest.responseJSON[BaseData.msg];
			    if(code == BaseData.noLogin) {
				    //需要登录
				    this.login.call(this, XMLHttpRequest.responseJSON, textStatus, XMLHttpRequest);
			    } else {
				    if(this.exeFail !== false && typeof this.fail == "function") {
					    this.fail.call(this, XMLHttpRequest.responseJSON, textStatus, XMLHttpRequest);
				    }
				    if (this.tips !== false) {
					    (this.showType == "alert" ? $.alert : $.msg)("error", msg ? msg : this.failMsg ? this.failMsg : "操作失败！");
				    }
			    }
		    }
		}
    ...
    $.ajaxPrefilter(function(opts, originalOptions, jqXHR ) {
    	...
    	opts.success = function(data, textStatus, jqXHR) {
			...
			if(data instanceof Object && data[BaseData.code] != null) {
				var code = data[BaseData.code],
					msg = data[BaseData.msg];
	 ...
	 */
	String.EMPTY_INDEX_MODEL 	= 1;
	/**
	 * 定位参数数组的索引值格式：{0} - 花括号里面为数字，即为对应的索引值
	 */
	String.NUM_INDEX_MODEL 		= 2;
	/**
	 * 定位参数值的格式：{key} - 花括号里面为key，依次遍历参数数组，找出Map和VO对象，取对应key的值
	 */
	String.KEY_VALUE_MODEL 		= 4;
	/**
	 * 默认配置 = {
	 * 		//三种值得组合：具体解释请看常量
	 * 		model: EMPTY_INDEX_MODEL | NUM_INDEX_MODEL | KEY_VALUE_MODEL,
	 * 		//当没有对应的参数或参数为null时，默认代替的值。defaultVal:null - 不做任何值替换；defaultVal:"" - 空字符窜代替；defaultVal:"null" - "null"字符窜代替
	 * 		defaultVal: ""
	 * }
	 */
	String.prototype.format = function () {
        ...
        while ((key = nextKey()) != null) {
            ...
            if (val == null && (opts.model & String.EMPTY_INDEX_MODEL) > 0 && !key.trim()) {
				index++;
				val = args.length > index ? args[index] : null;
			} 
			if (val == null && (opts.model & String.NUM_INDEX_MODEL) > 0 && utils.regex(key, regex.digit)) {
				var intKey = parseInt(key);
				val = args.length > intKey ? args[intKey] : null;
			}
			insertBack(val == null ? opts.defaultVal == null ? open + key + close : opts.defaultVal : val);
        }
        return assemble;
    };
    //截取一定字符的字符串
